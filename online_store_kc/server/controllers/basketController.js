const {Basket, BasketProducts, Product, ProductInfo} = require('./../models/models');
...
class BasketController {
    async addProduct(req, res) {
        try {
            ...
            await BasketProducts.create({basketId : basket.id, productId: id});
            return res.json("Товар добавлен в карточку");
        } catch (e) {
            console.error(e);
        }
    }
    ...
    async getProducts(req, res) {
        try {
            ...
            const basket = await BasketProducts.findAll({where: {basketId: id}});
        ...
        try {
            ...
            await Basket.findOne({where: {userId: user.id}}).then(async userBasket => {
                if(userBasket.userId === user.id) {
                    await BasketProducts.destroy({where: {basketId: userBasket.id, productId: id}})
                }
